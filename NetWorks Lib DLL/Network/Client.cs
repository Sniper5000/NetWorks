...
class Client
{
    ...
    public void Connect(string hostname, int port)
    {
        ...
        PacketProtocol.Send(tcpClient.GetStream(), JsonSerializer.SerializeToUtf8Bytes(new ClientHandshakeData(Keys.PublicKey.XmlString, udpPort)));
        // TODO hardcoded
        const int rxLimit = 4096;
        PacketProtocol.Receive(tcpClient.GetStream(), rxLimit, out int _, out byte[]? handshakeData);
        ServerHandshakeData handshake = JsonSerializer.Deserialize<ServerHandshakeData>(handshakeData)
